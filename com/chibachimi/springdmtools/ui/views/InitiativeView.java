...
public class InitiativeView extends VerticalLayout {
    ...
    private final DiceLogic diceLogic = new DiceLogic();
    ...
    public InitiativeView() {
        ...
        Grid.Column<Character> nameColumn = initGrid.addColumn(Character::getName)
                .setHeader("Character");
        Grid.Column<Character> bonusColumn = initGrid.addColumn(Character::getBonusAsString)
                .setHeader("Bonus");
        Grid.Column<Character> rollColumn = initGrid.addColumn(Character::getRollAsString)
                .setHeader("Roll")
                .setSortable(true)
        ...
        binder.forField(bonusField)
                .asRequired("This character needs a bonus!")
                .bind(Character::getBonusAsString, Character::setBonusAsString);
        bonusColumn.setEditorComponent(bonusField);
        ...
        // TODO Make sure this works with new dice system
        GridContextMenu<Character> menu = initGrid.addContextMenu();
        menu.addItem("Quick Roll", e->{
            e.getItem().ifPresent(item -> {
                item.setRollAsString(
                        diceLogic.rollDice(1, 20, item.getBonus())
                );
            });
            initGrid.getDataProvider().refreshAll();
        });
        menu.addItem("Advantage", e-> {
            e.getItem().ifPresent(item -> {
                item.setRollAsString(
                        diceLogic.rollAdv(item.getBonus())
                );
            });
            initGrid.getDataProvider().refreshAll();
        });
        menu.addItem("Disadvantage", e-> {
            e.getItem().ifPresent(item -> {
                item.setRollAsString(
                        diceLogic.rollDis(item.getBonus())
                );
            });
            initGrid.getDataProvider().refreshAll();
        });
